...
public class WaterLeviathan : MonoBehaviour, IItemOfInterest, IHostile
{
	...
	public IBodyOfWater TargetSource;
	...
	public virtual float SeekSpeed {
		get {
			return Globals.LeviathanMoveSpeed;
		}
	}
	...
	protected void BlowBubbles ()
	{
		...
		if (TargetSource != null) {
			bubblesPosition.y = TargetSource.WaterHeightAtPosition (transform.position);
		} else {
	...
	public virtual void LateUpdate ()
	{
		if (Wake.enableEmission) {
			...
			if (TargetSource != null) {
				wakePosition.y = TargetSource.WaterHeightAtPosition (wakePosition);
			} else {
				wakePosition.y = Ocean.Get.OceanSurfaceHeight;
			}
			Wake.transform.position = wakePosition;
			//Debug.Log ("Setting wake position to " + wakePosition.ToString ());
		}
	}

	protected virtual IEnumerator CustomDormantAction () {
		yield return null;
		yield break;
	}
		...
		while (GameManager.State != FGameState.Quitting) {
			...
			case HostileMode.Dormant:
				...
				if (HasPrimaryTarget) {
					...
					IEnumerator customAction = CustomDormantAction ();
					while (customAction.MoveNext ()) {
						yield return customAction.Current;
					}
				}
				break;
	...
	protected IEnumerator StalkTargetOverTime ()
	{
		...
		while (WorldClock.AdjustedRealTime < (startStalkTime + Globals.LeviathanRTMinimumStalkInterval)) {
			...
			Vector3 newPosition = Vector3.MoveTowards (transform.position, targetPosition, SeekSpeed).To2D ();
