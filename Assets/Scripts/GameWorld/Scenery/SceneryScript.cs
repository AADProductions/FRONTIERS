...
namespace Frontiers.World
{
	public class SceneryScript : MonoBehaviour
	{
		...
		public virtual bool InitializeOnEncounter {
			get {
				return true;
			}
		}
		...
		public virtual void OnPlayerEncounter ()
		{
			...
			if (InitializeOnEncounter && !string.IsNullOrEmpty (mSceneryState)) {
				UpdateSceneryState (mSceneryState);
			}
		}
		...
		public virtual void UpdateSceneryState (string sceneryState)
		{
			mSceneryState = sceneryState;
			if (InitializeOnEncounter && !HasBeenEncountered) {
				return;
			}
			//script props should have been updated by now
			if (mHasSceneryStateField && !string.IsNullOrEmpty (mSceneryState)) {
				object sceneryStateValue = null;
				try {
					if (XmlDeserializeFromString (mSceneryState, mSceneryStateField.FieldType, out sceneryStateValue)) {
		...
		protected string mSceneryState = string.Empty;
	}
