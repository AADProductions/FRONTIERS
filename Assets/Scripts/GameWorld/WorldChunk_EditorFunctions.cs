...
namespace Frontiers.World
{
	public partial class WorldChunk
	{
		...
		protected void GetTerrainData(ChunkTerrainData terrainData, Terrain terrain)
		{
			...
			//re-arrange plant instance prefabs
			for (int i = 0; i < terrain.terrainData.treePrototypes.Length; i++) {
				if (terrain.terrainData.treePrototypes[i].prefab.name.Equals("PlantInstancePrefab")) {
					//bump all instances down
					plantInstancePrototypeIndex = i;
				}
			}

			if (plantInstancePrototypeIndex >= 0 && plantInstancePrototypeIndex != terrain.terrainData.treePrototypes.Length - 1) {
				int newIndex = 0;
				TreePrototype[] oldTreePrototypes = terrain.terrainData.treePrototypes;
				TreePrototype[] newTreePrototypes = new TreePrototype[oldTreePrototypes.Length];
				Dictionary <int,int> changedPrototypes = new Dictionary<int, int>();
				for (int i = 0; i < oldTreePrototypes.Length; i++) {
					if (i == plantInstancePrototypeIndex) {
						changedPrototypes.Add(i, oldTreePrototypes.Length - 1);
						//don't increment the new index for this spot
					} else {
						newTreePrototypes[newIndex] = oldTreePrototypes[i];
						if (i > plantInstancePrototypeIndex) {
							changedPrototypes.Add(i, newIndex);
						}
						newIndex++;
					}
				}
				newTreePrototypes[oldTreePrototypes.Length - 1] = oldTreePrototypes[plantInstancePrototypeIndex];
				int pIndex = 0;
				Debug.Log("---OLD---");
				foreach (TreePrototype t in oldTreePrototypes) {
					Debug.Log("Old tree prototype " + pIndex.ToString() + ": " + (t == null ? " (NULL) " : t.prefab.name));
					pIndex++;
				}
				pIndex = 0;
				Debug.Log("---NEW---");
				foreach (TreePrototype t in newTreePrototypes) {
					Debug.Log("New tree prototype " + pIndex.ToString() + ": " + (t == null ? " (NULL) " : t.prefab.name));
					pIndex++;
				}
				terrain.terrainData.treePrototypes = newTreePrototypes;

				TreeInstance[] oldTreeInstances = terrain.terrainData.treeInstances;
				for (int i = 0; i < oldTreeInstances.Length; i++) {
					TreeInstance t = oldTreeInstances[i];
					if (changedPrototypes.ContainsKey(t.prototypeIndex)) {
						t.prototypeIndex = changedPrototypes[t.prototypeIndex];
						oldTreeInstances[i] = t;
					}
				}
				terrain.terrainData.treeInstances = oldTreeInstances;
