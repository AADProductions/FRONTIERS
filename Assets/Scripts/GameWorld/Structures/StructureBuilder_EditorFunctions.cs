...
namespace Frontiers.World
{
	public partial class StructureBuilder : Builder
	{
		...
		public static void AddChildrenToStructureTemplateGroup (Transform start, StructureTemplateGroup templateGroup)
		{
			...
			StructureTemplate.AddMovementNodesToStructureTemplate (startNodes, startStat, templateGroup.MovementNodes);
		}
...
//					ccg.centerColor = gizmoColor;
			...
			for (int i = 0; i < templateGroup.UniqueWorlditems.Count; i++) {
				...
				StackItem stackItem = templateGroup.UniqueWorlditems [i];
				WorldItem packPrefab = null;
				if (WorldItems.Get.PackPrefab (stackItem.PackName, stackItem.PrefabName, out packPrefab)) {
					...
					if (basePrefab != null) {
						...
						instantiatedWorldItem.ReceiveState (ref stackItem);
					} else {
			...
			foreach (MovementNode movementNode in templateGroup.MovementNodes) {
				GameObject newAcp = new GameObject ("ACP");
				ActionNodePlaceholder acp = newAcp.AddComponent <ActionNodePlaceholder> ();
				newAcp.transform.parent = startNodes;
				newAcp.transform.localPosition = new Vector3 (movementNode.X, movementNode.Y, movementNode.Z);
				acp.Index = movementNode.Index;
				acps.Add (acp);
			}
			ActionNodePlaceholder.LinkPlaceholders (acps, templateGroup.MovementNodes);
