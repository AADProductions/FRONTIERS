...
namespace Frontiers.World
{
		public class LiquidContainer : WIScript
		{
				...
				{
						if (State.IsEmpty) {
								GUIListOption option = new GUIListOption("(Empty)", "Drink");
								option.Disabled = true;
								options.Add(option);
						} else {
								if (Player.Local.Status.HasCondition("BurnedByFire")) {
										options.Add(new GUIListOption("Pour on Self", "PourOnSelf"));
								}
								options.Add(new GUIListOption("Drink " + State.Contents.DisplayName, "Drink"));
								options.Add(new GUIListOption("Pour Out"));
						}
				}
