...
namespace Frontiers.World
{
	public partial class WorldItem
	{
			...
			get {
				...
				if (Application.isPlaying) {
					...
					while (systemType.MoveNext ()) {
						...
						mScriptNames.Add (systemType.Current.Name);
					}
					#if UNITY_EDITOR
				} else {
					...
					foreach (WIScript script in scripts) {
						...
						mScriptNames.Add (script.ScriptName);
					}
				}
				#endif
				return mScriptNames;
		...
		protected HashSet<string> mScriptNames;
		...
		public void ReceiveState (ref StackItem newState)
		{
			...
			if (Is (WILoadState.Initialized) || IsTemplate) {
				...
				HashSet <string> scriptNames = ScriptNames;
				...
				var namesEnum = scriptNames.GetEnumerator ();
				while (namesEnum.MoveNext ()) {
					//for (int i = 0; i < scriptNames.Count; i++) {
					if (!newState.SaveState.Scripts.ContainsKey (namesEnum.Current)) {
						//get the script and finish it
						//this will automatically remove it
						WIScript wiScript = null;
						if (Get (namesEnum.Current, out wiScript)) {
							wiScript.Finish ();
						}
					}
				}
				...
				SaveState.CopyFrom (newState.SaveState);
			}
			//we're done!
			//unlock the world item
			mLockSaveState = false;
		...
		public void Initialize ()
		{
			...
			try {
				...
				if (checkSaveState) {
					HashSet <string> currentScriptNames = ScriptNames;
		...
		public IEnumerator GetSaveStateOverTime (WISaveState saveState)
		{
			var enumerator = mScripts.Values.GetEnumerator ();
			while (enumerator.MoveNext ()) {
				try {
		...
		public IEnumerator GetStackItemOverTime (StackItem newStackItem, WIMode stackItemMode)
		{
			//get the latest updated names before we copy them
			RefreshNames (true);
			//get the latest position in local props before we copy them
			...
			newStackItem.Name = Props.Name.FileName;
			if (newStackItem.SaveState == null) {
				newStackItem.SaveState = new WISaveState ();
			}
			var getSaveStateEnum = GetSaveStateOverTime (newStackItem.SaveState);
			while (getSaveStateEnum.MoveNext ()) {
				yield return getSaveStateEnum.Current;
			}
			yield break;
		}
		#if UNITY_EDITOR
