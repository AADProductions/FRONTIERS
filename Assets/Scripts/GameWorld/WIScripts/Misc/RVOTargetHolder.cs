...
public class RVOTargetHolder : MonoBehaviour
{
		...
		public float OuterCircleMinDistance = 6.5f;
		public float OuterCircleMaxDistance = 8.0f;
				...
				if (mGroundStalkers.Count > 0) {
						...
						for (int i = mGroundStalkers.Count - 1; i >= 0; i--) {
								...
								if (mGroundStalkers[i] == null || mGroundStalkers[i].parent != tr) {	//get rid of it
										...
										SpotHelper.position = tr.position + (Vector3.forward * targetDistance);
										SpotHelper.rotation = Quaternion.identity;
										if (i % 2 != 0) {//if it's an odd number, rotate normally
												SpotHelper.RotateAround(tr.position, Vector3.up, (float)(targetRotate * WorldClock.AdjustedRealTime));//then rotate it
										} else {//if it's an even number reverse rotation
												SpotHelper.RotateAround(tr.position, Vector3.up, (float)-(targetRotate * WorldClock.AdjustedRealTime));
										}
