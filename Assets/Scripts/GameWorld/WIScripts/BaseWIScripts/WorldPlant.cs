...
namespace Frontiers.World.WIScripts
{
	...
	{
		public FlowerSeekingCritter OccupyingCritter;

		public bool IsOccupied {
			get {
				if (OccupyingCritter != null) {
					if (OccupyingCritter.Destroyed || OccupyingCritter.CurrentFlower != this) {
						OccupyingCritter = null;
					}
				}
				return OccupyingCritter != null;
			}
		}
...
//			}
		...
		public Vector3 GetRandomFlowerPosition ()
		{
			if (HasPlantProps && Props.HasFlowers && Props.FlowerPositions.Count > 0) {
				return worlditem.tr.TransformPoint (Props.FlowerPositions [UnityEngine.Random.Range (0, Props.FlowerPositions.Count)]);
			}
