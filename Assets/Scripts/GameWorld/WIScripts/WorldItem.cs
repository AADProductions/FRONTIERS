...
namespace Frontiers.World
{
	public partial class WorldItem : MonoBehaviour, IWIBase, IUnloadableChild, IVisible
	{
		...
		public int LastActiveDistanceUpdate = 0;

		public float LastActiveDistanceToPlayer {
			set {
				mDistanceToPlayer = Mathf.Clamp (value - ActiveRadius, 0f, Mathf.Infinity);
			}
		}
		...
		protected float mDistanceToPlayer = -1f;
		...
		public bool HasPlayerFocus {
			get {
				...
				if (value) {
					...
					gameObject.GetOrAdd <PlayerFocusHighlight> ();
					RefreshHud ();
