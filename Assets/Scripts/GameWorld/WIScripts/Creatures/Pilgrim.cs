...
//								GUIListOption giveDirections = new GUIListOption("SkillIconGuildFollowTrail", "Give Directions", "GiveDirections");
			...
			while (State.PathMode != FollowPathMode.None) {	//check the state of the motile action we submitted
				switch (mFollowPathAction.State) {
					...
					case MotileActionState.Starting:
						...
						yield return StartCoroutine(UpdateFollowPath());
				...
				//wait for a bit
				yield return new WaitForSeconds(gPathFollowInterval);
			}
...
//						LastLocationReached = null;
			...
			if (State.TookDirection) {
				GUIManager.PostWarning(worlditem.DisplayName + " needs directions.");
			}
			yield break;
		}
