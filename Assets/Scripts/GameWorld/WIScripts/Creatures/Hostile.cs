...
namespace Frontiers.World
{
		...
		{		//used by both characters and creatures
				...
				public void Stalk()
				{
						...
						//yield return StartCoroutine (mStalkAction.WaitForActionToStart (0.1f));
				...
				public IEnumerator AttackImmediately()
				{
						...
						State.LastAttemptedAttackWTime = WorldClock.Time;
						startAction.SafeInvoke();
						DamagePackage damage = style.Damage;
						//wait until the attack is supposed to hit
						yield return new WaitForSeconds(style.RTPreAttackInterval);
						...
						yield return new WaitForSeconds(style.RTPostAttackInterval);
				...
				{
						mWarningOverTime = true;
						yield return StartCoroutine(mWarnAction.WaitForActionToStart(0.1f));
						OnWarn.SafeInvoke();
				...
				protected IEnumerator AttackOverTime()
				{
						...
						while (!mStalkAction.IsFinished && Mode == HostileMode.Attacking) {
								if (CanAttack) {
										yield return StartCoroutine(AttackImmediately());
								}
								yield return null;
						}
