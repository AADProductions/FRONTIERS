...
namespace Locomotion {
	...
	public class HeadLookController : MonoBehaviour {
		...
		public bool awareOfTarget			= true;
		public Vector3 currentEyeMovement;
		public float eyeMovementInterval	= 1.0f;
		public float eyeMovementMagnitude	= 4.0f;
		public float nextEyeMovement;
		...
		void Update ( )
		{
			if (!awareOfTarget)
			{
				target = leftEye.transform.position + rootNode.forward;
	//			//Debug.Log ("Not aware of target");
				return;
			}
			...
			{
				target = Player.Local.HeadPosition;
			}
		}
	...
	//        }
	    ...
	    void LateUpdate () {
			...
			{
				nextEyeMovement 	= Time.time + eyeMovementInterval + (Random.Range (1.0f, -1.0f) * eyeMovementInterval);
				currentEyeMovement 	= new Vector3 (Random.value, Random.value, Random.value) * eyeMovementMagnitude;
			}
			
			leftEye.transform.Rotate (currentEyeMovement);
			rightEye.transform.Rotate (currentEyeMovement);
