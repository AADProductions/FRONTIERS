...
public class DetonatorSound : DetonatorComponent {
	...
	void Update()
	{
		...
		{
			_explodeDelay = (_explodeDelay - (float) Frontiers.WorldClock.ARTDeltaTime);
			if (_explodeDelay <= 0f)
			{
				Explode();
			}
	...
	//		_soundComponent.rolloffFactor = rolloffFactor;
			...
			if (Vector3.Distance(Frontiers.Player.Local.Position, this.transform.position) < distanceThreshold)
			{
