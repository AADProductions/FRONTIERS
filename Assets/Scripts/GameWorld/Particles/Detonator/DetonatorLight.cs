...
public class DetonatorLight : DetonatorComponent {
	...
	void Update () 
	{
		...
		if ((_explodeTime + _scaledDuration > Frontiers.WorldClock.AdjustedRealTime) && _lightComponent.intensity > 0f)
		{
			_reduceAmount = intensity * (float) (Frontiers.WorldClock.ARTDeltaTime/_scaledDuration);
