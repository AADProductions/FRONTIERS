...
namespace Frontiers.World.WIScripts
{
	...
	public class PathAvatar : MonoBehaviour
	{
		...
		public List <Vector3> CachedSplinePositions = new List<Vector3>();

		public bool HasCachedSplinePositions {
			get {
				return CachedSplinePositions.Count > 0;
			}
		}

		public void GetCachedSplinePositions(List<Vector3> cachedPositions, float midParam, int maxPositions, out Vector3 pathStartPosition, out int startIndex)
		{	
			pathStartPosition = CachedSplinePositions[0];
			int midIndex = Mathf.FloorToInt(CachedSplinePositions.Count * midParam);
			startIndex = Mathf.Clamp(midIndex - (maxPositions / 2), 0, CachedSplinePositions.Count);
			int endIndex = Mathf.Clamp(midIndex + (maxPositions / 2), startIndex, CachedSplinePositions.Count);
			try {
				for (int i = startIndex; i < endIndex; i++) {
					cachedPositions.Add(CachedSplinePositions[i]);
				}
			} catch (Exception e) {
				Debug.Log("Start index: " + startIndex.ToString() + ", end index: " + endIndex.ToString() + ", mid param: " + midParam.ToString());
		...
		public void Refresh()
		{
			...
			int numCachedPositions = Mathf.FloorToInt(LengthInMeters * Globals.NumCachedSplinePositionsPerMeter);
			float metersPerPosition = LengthInMeters / numCachedPositions;
			for (int i = 0; i < numCachedPositions; i++) {
				CachedSplinePositions.Add(PositionFromMeters(metersPerPosition * i));
			}
		}
		...
		{
			return spline.GetClosestPointParam(position, 1, 0f, 1f, 0.01f) * (spline.Length * Globals.InGameUnitsToMeters);
		}
