...
namespace Frontiers
{
	public class Cutscene : MonoBehaviour
	{
		...
		public static void GetActiveInterfaceObjects(List<FrontiersInterface.Widget> currentObjects)
		{
			if (Cutscene.IsActive) {
				for (int i = 0; i < CurrentCutscene.InterfaceList.Count; i++) {
					Debug.Log("Getting cutscene objects from " + CurrentCutscene.InterfaceList[i].name);
					CurrentCutscene.InterfaceList[i].GetActiveInterfaceObjects(currentObjects);
				}
			}
		}
