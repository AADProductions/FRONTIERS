...
namespace Frontiers
{
	...
	public class StatusKeeper : Mod, IComparable <StatusKeeper>
	{
		...
		public void ApplyConditions (double deltaTime, List <AvatarAction> subscribedActions, List <Condition> activeConditions, List <string> activeStates)
		{
			...
			for (int i = Conditions.Count - 1; i >= 0; i--) {
				if (Conditions [i] == null || Conditions [i].HasExpired) {// Conditions [i].CheckExpired (deltaTime, subscribedActions, activeConditions, activeStates)) {
	...
	public class Condition : Mod
	{
		...
		public float HeatExposureDurationRatio = 0f;
		public float LightExposureDurationRatio = 0f;
		public float HeatExposureCureThreshold = -1f;
		public float LightExposureCureThreshold = -1f;
		                          ...
		                          List <string> activeStates)
		{
			...
			if (UpdateExposure (lightExposure, heatExposure, deltaTime)) {
				return true;
			}
			TimeSoFar += deltaTime;
		...
		protected bool UpdateExposure (float lightExposure, float heatExposure, double deltaTime)
		{
			double additionalTime = (heatExposure * HeatExposureDurationRatio * deltaTime) + (lightExposure * LightExposureDurationRatio * deltaTime);
			/*if (additionalTime > 0) {
				Debug.Log ("Light and heat added " + additionalTime.ToString ( ) + " to time so far in condition");
			}*/
			TimeSoFar += additionalTime;

			if (HeatExposureCureThreshold > 0 && heatExposure > HeatExposureCureThreshold) {
				Cancel ();
			}
			if (LightExposureCureThreshold > 0 && lightExposure > LightExposureCureThreshold) {
				Cancel ();
			}
			return mExpired;
		}
