...
namespace Frontiers.World.Gameplay
{
	...
	public class MissionObjective : MonoBehaviour
	{
		...
		public void ActivateObjective (ObjectiveActivation activation, MissionOriginType originType, string originName)
		{
			...
			if (!Flags.Check ((uint)State.Status, (uint)(MissionStatus.Active | MissionStatus.Completed), Flags.CheckType.MatchAny)) {	//activate and announce activation
				...
				if (!Flags.Check ((uint)State.Status, (uint)MissionStatus.Ignored, Flags.CheckType.MatchAny)) {	//if we HAVEN'T ignored this mission then we'll update
...
//			}
			...
			foreach (MissionObjective nextObjective in NextObjectives) {
				...
				if (!Flags.Check ((uint)nextObjective.State.Status, (uint)MissionStatus.Active, Flags.CheckType.MatchAny)
	...
	public class ObjectiveState
	{
		...
		[NonSerialized]
		protected List <ObjectiveState>
			mNextObjectiveStates = null;
