...
namespace Frontiers.World.Gameplay
{
	...
	[XmlInclude (typeof(ObjectivePlaceQuestItemInRecepticle))]
...
//	[XmlInclude (typeof (ObjectiveDestroyQuestCreature))]
	public class ObjectiveScript
	{
		[XmlIgnore]
		[NonSerialized]
		public MissionObjective objective = null;
	...
	public class ObjectiveRequireStructureLoadState : ObjectiveScript {
		...
		public void FinishChecking (IWIBase iwiBase)
		{
			...
			if (iwiBase.GetStateOf <Structure> (out structureStateObject)) {
				...
				HasCompleted = Flags.Check <StructureLoadState> (LoadState, structureState.LoadState, Flags.CheckType.MatchAll);
				if (HasCompleted) {
		...
		public override void CheckIfCompleted ()
		{
			...
			if (missionName == objective.mission.State.Name) {
				...
				if (objective.mission.GetObjective (ObjectiveName, out checkObjective)) {
					bool hasCompleted = Flags.Check <MissionStatus> (checkObjective.State.Status, MissionStatus.Completed, Flags.CheckType.MatchAny);
					bool hasFailed = Flags.Check <MissionStatus> (checkObjective.State.Status, MissionStatus.Failed, Flags.CheckType.MatchAny);
				...
				if (Missions.Get.ObjectiveStatusByName (missionName, ObjectiveName, ref objectiveStatus)) {
					bool hasCompleted = Flags.Check <MissionStatus> (objectiveStatus, MissionStatus.Completed, Flags.CheckType.MatchAny);
					bool hasFailed = Flags.Check <MissionStatus> (objectiveStatus, MissionStatus.Failed, Flags.CheckType.MatchAny);
					if (!hasCompleted || (hasFailed && !FailureOK)) {
						result = false;
					}
				}
	...
	{
		public string MissionName = string.Empty;//by default it's this mission
	...
	[Serializable]
	public class ObjectivePlaceQuestItemInRecepticle : ObjectiveScript
	{
		public string ItemName = "Item";
		public string RecepticleName = "Recepticle";
	...
	public class ObjectiveCheckBookStatus : ObjectiveScript
	{
		...
		public BookStatus CheckStatus = BookStatus.Dormant;
		...
		public override void CheckIfCompleted ()
		{
			...
			if (Books.GetBookStatus (BookName, out status)) {
				if (Flags.Check <BookStatus> (status, CheckStatus, Flags.CheckType.MatchAny)) {
					HasCompleted = true;
					Status |= MissionStatus.Completed;
				}
			}
