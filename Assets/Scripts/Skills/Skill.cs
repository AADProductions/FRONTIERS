...
namespace Frontiers.World.Gameplay
{
	...
	public class Skill : MonoBehaviour, IComparable <Skill>, IProgressDialogObject
	{
		...
		public virtual string DisplayName {
			get {
				...
				if (string.IsNullOrEmpty (Info.GivenName)) {
					return GUIManager.AddSpacesToText (name);
				}
		...
		public Color SkillIconColor {
			get {
				if (HasBeenLearned) {
					return Colors.BlendThree (Colors.Get.GenericLowValue, Colors.Get.GenericMidValue, Colors.Get.GenericHighValue, State.NormalizedUsageLevel);
				} else if (HasBeenDiscovered) {
					return Colors.Disabled (Colors.Get.SkillIconColor);
				} else {
					return Color.black;
				}
			}
	...
	{
		public int SkillLevel;//arbitrary skill level assigned for book distribution
	...
	public class SkillUsage
	{
		...
		public bool PassiveUsage = false;//skill is 'always on'
		public bool RealTimeDuration = false;//pauses when the game is paused
