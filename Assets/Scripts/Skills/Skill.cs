...
namespace Frontiers.World.Gameplay
{
	...
	public class Skill : MonoBehaviour, IComparable <Skill>, IProgressDialogObject
	{
		...
		public virtual float ProgressValue {
			get { return NormalizedEffectTimeSoFar; }
			set { }
		}
			...
			if (((result || !Usage.CanFail) || Usage.SendMessageOnFail)) {
				if (!string.IsNullOrEmpty (Usage.SendMessageToTargetObject)) {
					if (!string.IsNullOrEmpty (Usage.SendMessageArgument)) {
						#if DEBUG_SKILLS
						Debug.Log (name + ": Sending message " + Usage.SendMessageToTargetObject + " with argument " + Usage.SendMessageArgument);
						#endif
						...
						#if DEBUG_SKILLS
						Debug.Log (name + ": Sending message " + Usage.SendMessageToTargetObject + " with no argument");
						#endif
		...
		public virtual WIListOption GetListOption (IItemOfInterest targetObject)
		{
			if (!Usage.AppearInContextMenus) {
				#if DEBUG_SKILLS
				Debug.Log ("List Option: Skill " + name + " doesn't appear in context menus.");
				#endif
			...
			if (!DoesContextAllowForUse (targetObject)) {
				#if DEBUG_SKILLS
				Debug.Log ("List Option: Skill " + name + " context does not allow for use.");
				#endif
			...
			case SkillKnowledgeState.Unknown:
				#if DEBUG_SKILLS
				Debug.Log ("List Option: Skill " + name + " not valid.");
				#endif
				mListOption.IsValid = false;
				break;
			...
			get {
				...
				if (RequiresAtLeastOneEquippedWorldItem) {
					if (!Player.Local.Tool.IsEquipped) {
						#if DEBUG_SKILLS
						Debug.Log (name + ": No tool equipped, equipped requirements not met");
						#endif
						result = false;
					} else {
						...
						for (int i = 0; i < Requirements.RequiredEquippedWorldItems.Count; i++) {
							if (!Stacks.Can.Stack (
								...
								#if DEBUG_SKILLS
								Debug.Log (name + ": equipped item " + Player.Local.Tool.worlditem.name + " can't stack with " + Requirements.RequiredEquippedWorldItems [i] + ", equipped requirements not met");
								#endif
								result = false;
								break;
							}
						...
						if (!equippedWorldItem.HasAtLeastOne (Requirements.RequiredEquippedWIScriptNames)) {
							#if DEBUG_SKILLS
							Debug.Log (name + ": equipped item " + Player.Local.Tool.worlditem.name + " doesn't have at least one of the following:");
							foreach (string wiScript in Requirements.RequiredEquippedWIScriptNames) {
								Debug.Log (" - " + wiScript);
							}
							#endif
							result = false;
				...
				if (targetObject.gameObject.HasComponent <WorldItem> (out worlditem)) {
					for (int i = 0; i < Requirements.RequiredWIScriptNames.Count; i++) {
						...
						if (worlditem.Is (Requirements.RequiredWIScriptNames [i], out script)) {
							#if DEBUG_SKILLS
							Debug.Log (name + ": Target item " + worlditem.name + " has required script " + Requirements.RequiredWIScriptNames [i]);
							#endif
							if (!string.IsNullOrEmpty (Requirements.TargetRequirementCheckProperty)) {
								...
								if (propInfo != null) {
									...
									if (!requirementCheck) {
										#if DEBUG_SKILLS
										Debug.Log (name + ": Target script " + Requirements.RequiredWIScriptNames [i] + " doesn't have required prop " + Requirements.TargetRequirementCheckProperty);
										#endif
										return false;
									}
								}
							...
							#if DEBUG_SKILLS
							Debug.Log (name + ": Target item " + worlditem.name + " doesn't have required script " + Requirements.RequiredWIScriptNames [i]);
							#endif
							return false;
						}
					}
				...
				if (RequiresWorldItems) {
					for (int i = 0; i < Requirements.RequiredWorldItemNames.Count; i++) {
						if (!Player.Local.Inventory.HasItem (Requirements.RequiredWorldItemNames [i])) {
							#if DEBUG_SKILLS
							Debug.Log (name + " requires worlditem " + Requirements.RequiredWorldItemNames [i] + " and player doesn't have it, WorldItemRequirements not met");
							#endif
							return false;
						}
					}
	...
	public class SkillEffects
	{
		...
		[FrontiersFXAttribute]
		public string
			FXOnSuccessMasteredBooster;
		[FrontiersFXAttribute]
		public string
			FXOnFailure;
		[FrontiersFXAttribute]
		public string
			FXOnFailureCriticalBooster;
	}
	...
	public class SkillRequirements
	{
		...
		[FrontiersBitMask("GuildCredentials")]
		//TODO fix this (somehow?)
		public int
			RequiredCredentials = 0;
		...
		[FrontiersAvailableModsAttribute("StatusKeeper")]
		public string
			RequiredStatusType = string.Empty;
