...
namespace Frontiers.World.Gameplay
{
	...
	public class CraftSkill : Skill
	{
			...
			while (currentItem < totalItems && !mCancelled) {
				...
				if (!ConsumeRequirements (rows)) {
					...
					while (WorldClock.RealTime < endTime && !ProgressCanceled) {
						//yikes
						mProgressValue = (float) ((((WorldClock.RealTime - startTime) / (endTime - startTime)) * timeMultiplier) + timeOffset);//make sure it doesn't accidentally stop
		...
		protected void LoadBlueprintRow (List <InventorySquareCrafting> row, List <GenericWorldItem> requirements, BlueprintStrictness strictness)
		{
			for (int i = 0; i < row.Count; i++) {
				if (requirements [i] == null || requirements [i].IsEmpty) {
					row [i].EnabledForBlueprint = false;
					row [i].RequiredItemTemplate = null;
				} else {
					row [i].EnabledForBlueprint = true;
					row [i].RequiredItemTemplate = requirements [i];
					row [i].Strictness = strictness;
				}
		...
		public static bool AreRequirementsMet (IWIBase item, GenericWorldItem template, BlueprintStrictness strictness, int numItemsInStack, out int maxItemsToCraft)
		{
			...
			if (Flags.Check <BlueprintStrictness> (strictness, BlueprintStrictness.PrefabName, Flags.CheckType.MatchAny)) {
				prefabReqsMet = string.Equals (mCheckRequirements.PrefabName, template.PrefabName);
			}
			if (Flags.Check <BlueprintStrictness> (strictness, BlueprintStrictness.StackName, Flags.CheckType.MatchAny)) {
				stackReqsMet = Stacks.Can.Stack (mCheckRequirements.StackName, template.StackName);
			}
			if (Flags.Check <BlueprintStrictness> (strictness, BlueprintStrictness.StateName, Flags.CheckType.MatchAny)) {
				stateReqsMet = string.Equals (mCheckRequirements.State, template.State);
			}
			if (Flags.Check <BlueprintStrictness> (strictness, BlueprintStrictness.Subcategory, Flags.CheckType.MatchAny)) {
		...
		protected static GenericWorldItem mCheckRequirements;// = new GenericWorldItem ();
