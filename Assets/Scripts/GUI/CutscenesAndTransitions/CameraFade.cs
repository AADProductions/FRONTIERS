...
namespace Frontiers.GUI
{
	public class CameraFade : MonoBehaviour
	{
		...
		{   
			// If delay is over...
			if (WorldClock.RealTime > instance.m_FadeDelay) {
				// If the current color of the screen is not equal to the desired color: keep fading!
				if (instance.m_CurrentScreenOverlayColor != instance.m_TargetScreenOverlayColor) {			
					// If the difference between the current alpha and the desired alpha is smaller than delta-alpha * deltaTime, then we're pretty much done fading:
					if (Mathf.Abs(instance.m_CurrentScreenOverlayColor.a - instance.m_TargetScreenOverlayColor.a) < Mathf.Abs(instance.m_DeltaColor.a) * (float)Frontiers.WorldClock.RTDeltaTime) {
						...
						// Fade!
						SetScreenOverlayColor(instance.m_CurrentScreenOverlayColor + instance.m_DeltaColor * (float)Frontiers.WorldClock.RTDeltaTime);
					}
				}
			}
			// Only draw the texture when the alpha value is greater than 0:
			if (m_CurrentScreenOverlayColor.a > 0) {
				//i seriously don't care how much of a kludge this is i just want to get this working and go to sleep
				if (VRManager.VRMode) {
					VRManager.Get.FadeQuad.enabled = true;
					VRManager.Get.FadeQuad.material.color = m_CurrentScreenOverlayColor;
				} else {
		...
		/// Fade duration.
		/// </param>
		public static void StartAlphaFade(Color newScreenOverlayColor, bool isFadeIn, double fadeDuration)
		{
			if (fadeDuration <= 0.0f) {
				...
				instance.m_DeltaColor = (instance.m_TargetScreenOverlayColor - instance.m_CurrentScreenOverlayColor) / (float)fadeDuration;	
			}
		}
		...
		/// Fade delay.
		/// </param>
		public static void StartAlphaFade(Color newScreenOverlayColor, bool isFadeIn, double fadeDuration, float fadeDelay)
		{
			if (fadeDuration <= 0.0f) {
				SetScreenOverlayColor(newScreenOverlayColor);
			} else {
				instance.m_FadeDelay = (float)(WorldClock.RealTime + fadeDelay);			
		...
		/// </param>
		public static void StartAlphaFade(Color newScreenOverlayColor, bool isFadeIn, double fadeDuration, float fadeDelay, Action OnFadeFinish)
		{
			if (fadeDuration <= 0.0f) {
				SetScreenOverlayColor(newScreenOverlayColor);
			} else {
		...
		public static void HoldAlphaFade(Color color)
		{
		...
		void Die()
		{
			...
			mInstance = null;
			if (VRManager.VRMode) {
				VRManager.Get.FadeQuad.enabled = false;
			}
			Destroy(gameObject);
		}
