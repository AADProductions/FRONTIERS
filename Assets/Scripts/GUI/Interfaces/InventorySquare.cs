...
namespace Frontiers.GUI
{
	public class InventorySquare : InventorySquareDisplay
	{
			...
			if (IsEnabled && HasStack) {
				...
				if (mStack.HasTopItem) {
					...
					if (topItem.IsWorldItem) {
						TempProps = topItem.worlditem.Props;
					} else {
						TempProps = topItem.GetStackItem (WIMode.Stacked).Props;
					}
		...
		public virtual void SetInventoryStackNumber ()
		{
			...
			if (ShowDoppleganger && Doppleganger != null) {
				Vector3 labelPosition = StackNumberLabel.transform.position;
				if (Doppleganger.renderer != null) {
					labelPosition.z = -(Doppleganger.renderer.bounds.extents.z + 0.1f);
				}
		...
		{	//this whole method is very brittle
			...
			optionsList.ScreenTargetCamera = NGUICamrea;
		...
		public static bool UsingSkillList = false;
