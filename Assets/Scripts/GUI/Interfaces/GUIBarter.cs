...
namespace Frontiers.GUI
{
	public class GUIBarter : GUIEditor <BarterSession>, IInfoDisplay
	{
		...
		{
			PlayerGoodsBank.Session = EditObject;
			CharacterGoodsBank.Session = EditObject;
			CharacterInventoryBank.Session = EditObject;
			PlayerInventoryBank.Session = EditObject;

			BaseValueCharacterGoodsLabel.text = string.Empty;
			BaseValuePlayerGoodsLabel.text = string.Empty;

			//don't bother with the stack containers and squares
			//they take care of themselves
			if (mEditObject.BarteringCharacter.State.Flags.Gender == 1) {
				if (EditObject.NumCharacterGoods > 0) {
					BaseValueCharacterGoodsLabel.text = "Lowest price he'll accept for his goods:" + Colors.ColorWrap (" $", Colors.Darken (BaseValueCharacterGoodsLabel.color)) + mEditObject.TotalValueCharacterGoods.ToString ();
				} else {
					BaseValueCharacterGoodsLabel.text = "His price:" + Colors.ColorWrap (" $", Colors.Darken (BaseValueCharacterGoodsLabel.color)) + mEditObject.TotalValueCharacterGoods.ToString ();
				}
				if (EditObject.NumPlayerGoods > 0) {
					BaseValuePlayerGoodsLabel.text = "Highest price he'll offer for your goods:" + Colors.ColorWrap (" $", Colors.Darken (BaseValuePlayerGoodsLabel.color)) + mEditObject.TotalValuePlayerGoods.ToString ();
				} else {
					BaseValuePlayerGoodsLabel.text = "Your offer:" + Colors.ColorWrap (" $", Colors.Darken (BaseValuePlayerGoodsLabel.color)) + mEditObject.TotalValuePlayerGoods.ToString ();
				}
			} else {
				if (EditObject.NumCharacterGoods > 0) {
					BaseValueCharacterGoodsLabel.text = "Lowest price she'll accept for his goods:" + Colors.ColorWrap (" $", Colors.Darken (BaseValueCharacterGoodsLabel.color)) + mEditObject.TotalValueCharacterGoods.ToString ();
				} else {
					BaseValueCharacterGoodsLabel.text = "Her price:" + Colors.ColorWrap (" $", Colors.Darken (BaseValueCharacterGoodsLabel.color)) + mEditObject.TotalValueCharacterGoods.ToString ();
				}
				if (EditObject.NumPlayerGoods > 0) {
					BaseValuePlayerGoodsLabel.text = "Highest price she'll offer for your goods:" + Colors.ColorWrap (" $", Colors.Darken (BaseValuePlayerGoodsLabel.color)) + mEditObject.TotalValuePlayerGoods.ToString ();
				} else {
					BaseValuePlayerGoodsLabel.text = "Your offer:" + Colors.ColorWrap (" $", Colors.Darken (BaseValuePlayerGoodsLabel.color)) + mEditObject.TotalValuePlayerGoods.ToString ();
				}
			}
