...
namespace Frontiers.GUI
{
	public class GUISaveGameDialog : GUIBrowserSelectView <PlayerGame>
	{
		...
		public override void PushSelectedObjectToViewer()
		{
			...
			GameCreateResult.label.text = mSelectedObject.Name;
			RefreshSelection();
		}
		...
		public void RefreshSelection()
		{
			...
			if (Profile.Get.ValidateExistingGameName(Profile.Get.CurrentGame.WorldName, GameCreateResult.text)) {
				cleanAlternative = GameCreateResult.text;
				ValidExistingGame = true;
				Message.text = "This will override existing game " + GameCreateResult.text;
			} else if (Profile.Get.ValidateNewGameName(Profile.Get.CurrentGame.WorldName, GameCreateResult.text, out cleanAlternative)) {
				ValidNewGame = true;
				Message.text = string.Empty;
			}
			//CreateMessageLabel.text = createError;
			GameCreateResult.text = cleanAlternative;
			GameCreateResult.label.text = cleanAlternative;
