...
namespace Frontiers
{
	public class DebugConsole : MonoBehaviour
	{
		...
		public void Awake ()
		{
			...
			help.Add ("setcycle [numberOfMinutesPerInGameMinute]");
			...
			help.Add ("nosave (toggles no-save mode)");
			help.Add ("notrees (toggles no-trees mode, will take effect over time)");
		...
		public bool showWorldItems = false;
		...
		public void Update ()
		{
			if (!GameManager.Get.TestingEnvironment) {
				...
				if (GameWorld.Get.WorldLoaded) {
					...
					posLabelList.Add (WorldClock.AdjustedRealTime.ToString ("0.##") + " - Time of day: " + WorldClock.DayCycleCurrentNormalized.ToString ("0.##"));
					posLabelList.Add ("Hour cycle length: " + WorldClock.gHourCycleRT.ToString ("0.##"));
					posLabelList.Add ("Day cycle length: " + WorldClock.gDayCycleRT.ToString ("0.##"));
					posLabelList.Add ("Game Seconds per RT Second: " + WorldClock.gSecondsPerRTSecond.ToString ("0.#####"));
					posLabelList.Add ("Game Minutes per RT Second: " + WorldClock.gMinutesPerRTSecond.ToString ("0.#####"));
					posLabelList.Add ("Game Hours per RT Second: " + WorldClock.gHoursPerRTSecond.ToString ("0.#####"));
					posLabelList.Add ("RT Seconds per game second: " + WorldClock.gRTSecondsPerGameSecond.ToString ("0.#####"));
					posLabelList.Add ("RT Seconds per game minute: " + WorldClock.gRTSecondsPerGameMinute.ToString ("0.#####"));
					posLabelList.Add ("RT Seconds per game hour: " + WorldClock.gRTSecondsPerGameHour.ToString ("0.#####"));
					posLabelList.Add ("Day cycle length in RT seconds: " + WorldClock.gDayCycleRT.ToString ("0.##"));
					posLabelList.Add ("Day cycle length in RT minutes: " + WorldClock.SecondsToMinutes (WorldClock.gDayCycleRT).ToString ("0.##"));
		...
		{		//hahaha this function sucks so much
			...
			case "setcycle":
				WorldClock.SetCycleLength (Single.Parse (splitCommand [1]));
				break;

			case "nosave":
				GameManager.Get.NoSaveMode = !GameManager.Get.NoSaveMode;
				DebugConsole.Get.Log.Add ("#No save mode set to " + GameManager.Get.NoSaveMode.ToString ());
				break;

			case "notrees":
				GameManager.Get.NoTreesMode = !GameManager.Get.NoTreesMode;
				DebugConsole.Get.Log.Add ("#No trees mode set to " + GameManager.Get.NoTreesMode.ToString ());
				break;
			...
			case "spend":
				string amount = splitCommand [1].Trim ();
				int baseCurrency = Int32.Parse (amount);
				Player.Local.Inventory.InventoryBank.TryToRemove (baseCurrency);
				break;
					...
					if (splitCommand.Length < 2) {
						...
						for (int i = 0; i < numToAdd; i++) {
							if (WorldItems.Get.StackItemFromPack (packName, prefabName, out stackItem)) {
								...
								Debug.Log ("Couldn't get worlditem " + packName + ", " + prefabName + " for some reason");
							}
						}
