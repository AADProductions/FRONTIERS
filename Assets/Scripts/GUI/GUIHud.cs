...
namespace Frontiers.GUI
{
	public class GUIHud : MonoBehaviour
	{
		...
		public void ShowName (IItemOfInterest ioi, string newName) {
			for (int i = 0; i < GUINames.Length; i++) {
				if (GUINames [i].Name.IsEmpty) {
					GUINames [i].SetName (ioi, newName);
					break;
				}
			}
		}
		...
		//simple struct for displaying names on screen
		[Serializable]
		public struct HudName
		{
			public HudName (IItemOfInterest ioi, string name) {
				Name = name;
				Alpha = 0;
				IOI = ioi;
			}
			...
			public string Name;
			public float Alpha;
			public IItemOfInterest IOI;
			public bool IsEmpty {
				get {
					return string.IsNullOrEmpty (Name) || IOI == null || IOI.Destroyed;
				}
			}
