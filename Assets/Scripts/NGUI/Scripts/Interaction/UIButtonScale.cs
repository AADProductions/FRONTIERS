...
public class UIButtonScale : MonoBehaviour
{
	...
	void OnHover (bool isOver)
	{
		...
		{
			if (!mInitDone) Init();
			if (!isOver && tweenTarget.gameObject.HasComponent<UILabel>(out gCheckLabel)) {
					//refresh the local scale since it may have changed
								mScale = Vector3.one * gCheckLabel.font.defaultSize;
			}
		...
		protected static UILabel gCheckLabel;
}