...
public class UIPanel : MonoBehaviour
{
	...
	public Vector4 clipRange
	{
		...
		set
		{
			if (mClipRange != value)
			{
				mCullTime = (mCullTime == 0f) ? 0.001f : (float)(Frontiers.WorldClock.RealTime + 0.15);
				mCheckVisibility = true;
				mClipRange = value;
				UpdateDrawcalls();
			}
...
#if UNITY_EDITOR
		bool shouldCull = !Application.isPlaying || Frontiers.WorldClock.RealTime > mCullTime;
#else
		bool shouldCull = Frontiers.WorldClock.RealTime > mCullTime;
#endif
	...
	static public UIPanel Find (Transform trans, bool createIfMissing)
	{
		...
		{
			/*
			Debug.Log("Adding panel to " + trans.gameObject.name);
