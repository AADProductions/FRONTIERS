...
namespace Frontiers.Story.Conversations
{
	...
	public class Exchange : IComparable <Exchange>, IEqualityComparer<Exchange>
	{
		...
		public bool Disable {
			...
			set {
				#if DEBUG_CONVOS
				Debug.Log ("Exchange " + Name + " setting disabled to " + value.ToString ());
				#endif
		...
		public bool IsAvailable {
			get {
				if (!Enabled) {
					#if DEBUG_CONVOS
					Debug.Log ("Exchange IsAvailable: not enabled - Disabled (" + Disable.ToString () + ")");
					#endif
					return false;
				}
				...
				case AvailabilityBehavior.Once:
					...
					if (numTimesChosen > 0) {
						#if DEBUG_CONVOS
						Debug.Log ("Exchange IsAvailable: ONCE and chosen more than zero");
						#endif
						isAvailable = false;
					}
					break;
				...
				case AvailabilityBehavior.Max:
					if (NumTimesChosen >= MaxTimesChosen) {
						#if DEBUG_CONVOS
						Debug.Log ("Exchange IsAvailable: MAX (" + MaxTimesChosen.ToString () + ") and chosen (" + NumTimesChosen.ToString () + ")");
						#endif
						isAvailable = false;
					}
					break;
				...
				default:
					#if DEBUG_CONVOS
					Debug.Log ("Exchange IsAvailable: Always");
					#endif
					break;
				}
		...
		[NonSerialized]
		protected HashSet <Exchange>
			mPlayerOutgoingChoices = null;
		[NonSerialized]
		protected List <Exchange>
			mIncomingChoices = new List <Exchange> ();
		[NonSerialized]
		protected List <Exchange>
			mOutgoingChoices = null;
	}
}
