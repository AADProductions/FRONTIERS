...
namespace Frontiers
{
	public class UserKeyboardAndMouseProfile<T> : CustomInputDeviceProfile where T : struct, IConvertible, IComparable, IFormattable
	{
	...
	public class ActionSetting
	{
		...
		public bool AxisSetting {
			get {
				return Axis != InputAxis.None
					&& (Axis == InputAxis.MouseX || Axis == InputAxis.MouseY || Axis == InputAxis.ScrollWheel);
			}
		}
		...
		public bool HasOpposingControls (List <ActionSetting> settings)
		{
			if (OpposingActions != null) {
				if (OpposingControls == null || OpposingControls.Length != OpposingActions.Length) {
					List<InputControlType> opposingKeys = new List<InputControlType> ();
					for (int i = 0; i < OpposingActions.Length; i++) {
						for (int j = 0; j < settings.Count; j++) {
							if (settings [j].Action == settings [j].Action) {
								opposingKeys.Add (settings [j].Controller);
							}
						}
					}
					OpposingControls = opposingKeys.ToArray ();
				}
			}
			return OpposingControls != null;
		}

		public InputControlType[] OpposingControls;
		public int[] OpposingActions;
