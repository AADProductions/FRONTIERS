	...
	public class Plants : Manager
	{
		...
		public override void Initialize ()
		{
			...
			for (int i = 0; i < TerrainPlantPrototypes.Count; i++) {
				//turn off shadows for anything that isn't a tree
				if (!TerrainPlantPrototypes [i].HasComponent <TreeCollider> ()) {
					Renderer r = null;
					if (TerrainPlantPrototypes [i].HasComponent <Renderer> (out r)) {
						r.castShadows = false;
...
//														closestPlant.Climate = regionData.b;
		...
		protected IEnumerator BuildPlantSeasonalMesh (Plant plant, PlantSeasonalSettings seasonalSettings)
		{
			...
			if (mCurrentMeshOutputs != null && mCurrentMeshOutputs.Length > 0) {
				...
				Rigidbody rb = newPlantPrototype.AddComponent <Rigidbody> ();
				rb.isKinematic = true;
