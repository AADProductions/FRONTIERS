		...
		public static class Clear
		{
			...
			public static void DestroyedOrMovedItems (WIStack stack)
			{
				...
				for (int i = stack.Items.Count - 1; i >= 0; i--) {
					//Debug.Log ("Clearing item " + i.ToString ());
					mClearTopItem = stack.Items [i];
					if (mClearTopItem == null) {
						//Debug.Log ("STACKS: Removing item " + i + " from stack because it's null or destroyed");
						stack.Items.RemoveAt (i);
					} else if (mClearTopItem.Is (WIMode.RemovedFromGame)) {
						//Debug.Log ("Stack item " + mClearTopItem.DisplayName + " was removed from game: " + mClearTopItem.Mode);
						mClearTopItem.Clear ();
						stack.Items.RemoveAt (i);
					} else if (mClearTopItem.Group != stack.Group) {
						if (mClearTopItem.Group != null && stack.Group != null) {
							//Debug.Log ("Stack item group " + mClearTopItem.Group.name + " does not equal stack group " + stack.Group.name + ", removing");
						} else {
							//Debug.Log ("Stack item " + mClearTopItem.DisplayName + " does not belong to this stack's group, and one of them was null: " + (mClearTopItem.Group == null).ToString () + ", " + (stack.Group == null).ToString () + " removing");
						}
						stack.Items.RemoveAt (i);
					}
				}
