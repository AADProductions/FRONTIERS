		...
		public static class Push
		{
			...
			public static bool Item(WIStack stack, IWIBase item, bool toTop, StackPushMode pushMode, ref WIStackError error)
			{
				...
				if (Stacks.Can.Add(item, stack, ref error)) {
					...
					if (stack.Group == WIGroups.Get.Player) {
						int totalBaseCurrencyAdded = 0;
						int baseCurrencyValue = 0;
						if (item.IsQuestItem) {
						...
						if (item.IsStackContainer) {
							...
							for (int i = 0; i < item.StackContainer.StackList.Count; i++) {
								WIStack cStack = item.StackContainer.StackList[i];
								IWIBase c = null;
								for (int j = 0; j < cStack.Items.Count; j++) {
									c = cStack.Items[j];
									if (c != null && c.Is<Currency>()) {
										baseCurrencyValue = Mathf.CeilToInt(c.BaseCurrencyValue);
										totalBaseCurrencyAdded += baseCurrencyValue;
										Player.Local.Inventory.InventoryBank.Add(baseCurrencyValue, c.CurrencyType);
										c.RemoveFromGame();
									}
								}
							}
						...
						if (totalBaseCurrencyAdded > 0) {
							GUI.GUIManager.PostGainedItem(totalBaseCurrencyAdded, WICurrencyType.A_Bronze);
						}
					}
