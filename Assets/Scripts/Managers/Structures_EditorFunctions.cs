...
//								string prefabAssetPath = assetPath.Replace (".prefab", string.Empty);
				...
				public void DrawEditor()
				{
					...
					*/
						...
						for (int i = InteriorsWaitingToLoad.LastIndex(); i >= 0; i--) {
								Structure structure = InteriorsWaitingToLoad[i];
								if (structure != null) {
										if (GUILayout.Button(structure.name + " - " + structure.LoadState.ToString())) {
						...
						for (int i = MinorsWaitingToLoad.LastIndex(); i >= 0; i--) {
								MinorStructure structure = MinorsWaitingToLoad[i];
								if (structure != null) {
						...
						for (int i = ExteriorsWaitingToUnload.LastIndex(); i >= 0; i--) {
								Structure structure = ExteriorsWaitingToUnload[i];
								if (structure != null) {
										if (GUILayout.Button(structure.name + " - " + structure.LoadState.ToString())) {
						...
						for (int i = InteriorsWaitingToUnload.LastIndex(); i >= 0; i--) {
								Structure structure = InteriorsWaitingToUnload[i];
								if (structure != null) {
										if (GUILayout.Button(structure.name + " - " + structure.LoadState.ToString())) {
						...
						for (int i = MinorsWaitingToUnload.LastIndex(); i >= 0; i--) {
								MinorStructure structure = MinorsWaitingToUnload[i];
								if (structure != null) {
										if (GUILayout.Button(structure.TemplateName + " - " + structure.LoadState.ToString())) {
