...
namespace Frontiers {
	public class PlayerItemPlacement : PlayerScript
	{
		...
		public Recepticle PlacementPreferredRecepticle	= null;
		public RecepticlePivot PlacementPreferredRecepticlePivot = null;
		...
		public float ThrowForce = 8.0f;//TODO put this in globals
		...
		public bool PlacementInRecepticlePossible {
			get {
				return PlacementPreferredRecepticle != null;
			}
		}
		...
		{
			if (PlacementModeEnabled) {
				if (Player.Local.Surroundings.IsRecepticleInPlayerFocus) {
					//recepticles handle their own business
					Player.Local.Surroundings.RecepticleInPlayerFocus.worlditem.OnPlayerUse.SafeInvoke ();
					PlacementModeEnabled = false;
				} else {
					if (IsPlacingSomething) {
		...
		public bool ItemPickUp (WorldItem item)
		{
			...
			if (item.CanEnterInventory) {//quest items are automatically fine
				if (!item.Is <QuestItem> () && !item.Is <OwnedByPlayer> () && item.UseRemoveItemSkill (mRemoveItemSkillNames, ref mSkillUseTarget)) {
					...
					return true;
				} else if (Player.Local.Inventory.AddItems (item, ref error)) {
					//Debug.Log ("Adding item to inventory");
					return true;
				}
			} else {
				//Debug.Log ("Couldn't enter inventory");
			}
			return false;
		}
			...
			if (Player.Local.Tool.IsEquipped && ItemToPlace == Player.Local.Tool.worlditem) {
				...
				if (Player.Local.Inventory.PopAQIIntoWorld (out aqi, locationGroup, ref error)) {
					...
					if (PlacementResultsInDrop) {
						...
						ItemToPlace = null;
					} else if (PlacementInRecepticlePossible) {
						Debug.Log ("Adding item to recepticle");
						PlacementPreferredRecepticle.AddToRecepticle (ItemToPlace);
					} else {
		...
		protected void FindPreferredRecepticle ()
		{
			//order of prerefence:
			//placement of carried object
			//placement of equipped path editor
			//placement of equipped tool general
			PlacementPreferredRecepticle = null;
			PlacementPreferredRecepticlePivot = null;
			...
			if (IsCarryingSomething) {
				if (Player.Local.Surroundings.IsRecepticleInPlayerFocus) {
					Recepticle recepticle = Player.Local.Surroundings.RecepticleInPlayerFocus;
					if (recepticle.HasRoom (CarryObject, out PlacementPreferredRecepticlePivot) &&
						recepticle.IsObjectPermitted (CarryObject, PlacementPreferredRecepticlePivot.Settings)) {
						//we should affix the doppleganger to a pivot
						//the doppleganger color should be green
						PlacementPermitted = true;
						PlacementPreferredPoint = PlacementPreferredRecepticlePivot.tr.position;
						PlacementPreferredNormal = PlacementPreferredRecepticlePivot.tr.rotation.eulerAngles;
				...
				if (PlacementPermitted && !PlacementResultsInDrop)
				{	//first check the normal - the placement point may be on a wall or something
					if (Vector3.Dot (Vector3.up, PlacementPreferredNormal) < 0f)
					{	//turn off this collider before doing the raycast
			...
			if (Player.Local.Tool.IsEquipped) {
				...
				if (Player.Local.Surroundings.IsRecepticleInPlayerFocus) {
					Recepticle recepticle = Player.Local.Surroundings.RecepticleInPlayerFocus;
					if (	recepticle.HasRoom (Player.Local.Tool.worlditem, out PlacementPreferredRecepticlePivot) &&
						recepticle.IsObjectPermitted (Player.Local.Tool.worlditem, PlacementPreferredRecepticlePivot.Settings)) {
						//we should affix the doppleganger to a pivot
						//the doppleganger color should be green
						PlacementPreferredRecepticle = recepticle;
						PlacementPermitted = true;
						PlacementPreferredPoint = PlacementPreferredRecepticlePivot.tr.position;
						PlacementPreferredNormal = PlacementPreferredRecepticlePivot.tr.rotation.eulerAngles;
						PlacementResultsInDrop = false;
