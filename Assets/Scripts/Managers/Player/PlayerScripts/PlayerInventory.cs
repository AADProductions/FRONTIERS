...
namespace Frontiers
{
	public class PlayerInventory : PlayerScript, IInventory
	{
		...
		public override void OnLocalPlayerDespawn ()
		{
			...
			if (State.ActiveQuickslot < 0) {
				SelectionNext (WorldClock.RealTime);
			}
		}
		...
		public bool AddQuestItem (string questItem)
		{
		...
		public bool AddItems (IWIBase item, ref WIStackError error)
		{
			...
			if (!addResult) {
				...
				if (item.IsQuestItem) {
					if (State.QuestItemsAcquired.SafeAdd (item.QuestName)) {
						Player.Get.AvatarActions.ReceiveAction ((AvatarAction.ItemQuestItemAddToInventory), WorldClock.AdjustedRealTime);
						QuestItemState questItemState = null;
						if (item.IsWorldItem) {
							QuestItem questItem = item.worlditem.Get <QuestItem> ();
							questItemState = questItem.State;
						} else {
							StackItem questItem = item.GetStackItem (WIMode.Unloaded);
							questItem.GetStateData <QuestItemState> (out questItemState);
						}
						GUIManager.PostGainedItem (questItemState);
					}
				}
				Player.Get.AvatarActions.ReceiveAction ((AvatarAction.ItemAddToInventory), WorldClock.AdjustedRealTime);
			...
			if (item.IsStackContainer) {	
				...
				if (QuickslotEnabler.IsEnabled
					...
					for (int i = 0; i < quickslotStacks.Count; i++) {
						...
						if (!foundFirstCompatible && compatibleOK && !stack.IsFull && Stacks.Can.Stack (stack, item)) {
		...
		{
			Debug.Log ("Searching for quest item " + itemName + " and removing it from inventory");
			...
			foreach (WIStack stack in QuickslotEnabler.EnablerStacks) {
				for (int i = 0; i < stack.Items.Count; i++) {
					if (stack.Items [i] != null && stack.Items [i].QuestName.Equals (itemName)) {
			...
			if (questItem == null) {
				foreach (WIStackEnabler enabler in InventoryEnablers) {	//don't bother to check the enabler stack we already did that
					if (enabler.HasEnablerContainer) {
						foreach (WIStack stack in enabler.EnablerContainer.StackList) {
							for (int i = 0; i < stack.Items.Count; i++) {
								if (stack.Items [i] != null && stack.Items [i].QuestName.Equals (itemName)) {
									questItem = stack.Items [i];
									break;
			...
			if (questItem != null) {
				Debug.Log ("Found item, removing questitem from game");
				State.QuestItemsRemoved.SafeAdd (itemName);
				questItem.RemoveFromGame ();
			} else {
				Debug.Log ("Didn't find questitem");
			}
		}
		...
		public IEnumerator AddItems (WIStack stack, int numItems)
		{
			...
			for (int i = 0; i < numItems; i++) {
				//AddItems will drop items at feet if needed
				if (Stacks.Pop.TopIntoWorld (stack, out newWorldItem)) {
					AddItems (newWorldItem, ref error);
				} else {
					Stacks.Pop.Force (stack);
				}
				yield return null;
			}
			stack.Refresh ();
