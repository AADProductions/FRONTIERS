...
namespace Frontiers
{
    public class WIGroups : Manager
    {
        ...
        public override void Initialize() {     //we have a couple of groups that are always created by default
        ...
        public static void Load(WIGroup group) {
            ...
            switch (group.LoadState) {
                ...
                default:
                    Debug.Log("NOT adding load request for group " + group.name);
                    break;
            }
            //and that's it
        ...
        public static IEnumerator GetAllChildrenByType(string startGroup, List<string> wiScriptTypes, List<WorldItem> childrenOfType, Vector3 searchOrigin, float searchRadius, int maxItems) { //get all the live groups immediately below us
        ...
        protected IEnumerator GetAllStackItemsByTypeOverTime(string groupPath, List<string> wiScriptTypes, GroupSearchType searchType, Queue<StackItem> stackItemQueue) {   //start by getting all the paths to search for
