...
namespace Frontiers
{
	public partial class Structures : Manager
	{
		...
		//public MeshCombiner InteriorLODCombiner;
		...
		protected bool GetNextStructure(StructureLoadState desiredState, StructureLoadState precursorState, List <Structure> fromList, StructureLoadState disqualifiedState, out Structure structure)
		{
			...
			for (int i = fromList.LastIndex(); i >= 0; i--) {
				...
				if (nextStructure == null) {
					//Debug.Log ("GET NEXT: Structure was null - removing without building");
					fromList.RemoveAt(i);
					//if the structure has the right load state
				} else if (nextStructure.Is(desiredState) || nextStructure.Is(precursorState)) {
					//Debug.Log ("Next structure is " + desiredState.ToString () + " or " + precursorState.ToString () + ", proceeding");
					...
					if (nextStructure.Initialized) {
						...
						//Debug.Log ("GET NEXT: Structure " + nextStructure.name + " was not initialized: removing without building");
						//fromList.RemoveAt (i);
						//} else {
						//otherwise we're good to go
						//remove it from the list
						//Debug.Log ("GET NEXT: Structure " + nextStructure.name + " is ready to go for " + desiredState.ToString () + " - sending it over");
						fromList.RemoveAt(i);
						structure = nextStructure;
						//}
					}
				} else if (nextStructure.Is(disqualifiedState) && nextStructure.LoadPriority != StructureLoadPriority.SpawnPoint) {
					//Debug.Log ("GET NEXT: Structure load state WAS " + disqualifiedState.ToString () + ", it was " + nextStructure.LoadState.ToString ( ) + ", and this isn't wait for state " + desiredState.ToString ( ) + " - removing");
					fromList.RemoveAt(i);
				}
				break;
			}
			...
			StructureBuilder withBuilder)
		{
			...
			if (GetNextStructure(desiredState, precursorState, fromList, disqualifiedState, out structure)) {
				...
				//Debug.Log ("LOAD STRUCTURE OF TYPE " + withBuilder.Mode.ToString () + " - " + structure.name);
				if (Mods.Get.Runtime.LoadMod(ref mainTemplate, "Structure", structure.State.TemplateName)) {
					...
					//Debug.Log ("STRUCTURES - STARTED BUILDING " + structure.name);
					...
					if (withBuilder.State != StructureBuilder.BuilderState.Error && structure.Is(intermediateState)) {
						...
						//Debug.Log ("There was an error building structure " + structure.name + " - builders state is " + withBuilder.State.ToString () + " and structure load state is " + structure.LoadState.ToString ());
					}
					...
					yield return null;
					//Debug.Log ("STRUCTURES - FINISHED BUILDING " + structure.name);
				}
				yield return null;
			}
			yield break;
		...
		protected IEnumerator LoadMinorStructure(MinorStructure minorStructure)
		{
			...
			if (minorStructure != null && minorStructure.LoadState == StructureLoadState.ExteriorWaitingToLoad) {
				...
				if (Mods.Get.Runtime.LoadMod(ref template, "Structure", minorStructure.TemplateName)) {
					//Debug.Log ("STRUCTURES - STARTING MINOR STRUCTURE " + minorStructure.TemplateName);
					...
					//TODO see if this is really necessary
					System.GC.Collect(0, System.GCCollectionMode.Forced);
					//Debug.Log ("STRUCTURES - FINISHED MINOR STRUCTURE " + minorStructure.TemplateName);
