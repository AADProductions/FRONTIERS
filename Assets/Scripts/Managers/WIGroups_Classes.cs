...
namespace Frontiers
{
	...
	public class WIGroupLoadRequest : IComparable <WIGroupLoadRequest>, IEquatable <WIGroupLoadRequest>
	{
		...
		public int CompareTo (WIGroupLoadRequest other)
		{
			...
			int compareTo = 0;
			if (!string.IsNullOrEmpty (GroupPath) && !string.IsNullOrEmpty (other.GroupPath)) {
				compareTo = other.GroupPath.Length.CompareTo (GroupPath.Length);
			}
