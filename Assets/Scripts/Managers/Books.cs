...
namespace Frontiers
{
	public class Books : Manager
	{
		...
		public override void OnModsLoadFinish ()
		{
			...
			StartCoroutine (BuildBookTemplates ());
		...
		public void DeliverBookOrder (string libraryName) {
			...
			if (HasPlacedOrder (libraryName, out order) && !order.HasBeenDelivered) {
				order.WTPickUpTime = WorldClock.Time;
		...
		public bool TryToPlaceBookOrder (string libraryName, LibraryCatalogueEntry order, out string error) {
			...
			order.WTOrderedTime = WorldClock.Time;
			return true;
		}
		...
		protected IEnumerator BuildBookTemplates ()
		{	//this isn't necessary any more
			//but it may become necessary again later so i'm leaving it intact
//			GameObject templates = gameObject.FindOrCreateChild ("BookTemplatePrototypes").gameObject;
//			templates.transform.localPosition = new Vector3 (500, -500, -500);//keep it out of the way
//			//for each template, build one copy of each kind of book
//			//that way we don't have to build them on the fly later
//			for (int i = 0; i < Templates.Count; i++) {
//				BookTemplate template = Templates [i];
//				for (int j = 0; j < template.BookNames.Count; j++) {
//					string bookName = template.BookNames [j];
//					GameObject bookTemplatePrototype = templates.FindOrCreateChild (template.Name + "_" + bookName).gameObject;
//					bookTemplatePrototype.transform.localPosition = new Vector3 (i, 0f, j);
//					InitializeBookAvatarGameObject (bookTemplatePrototype, bookName, template.Name);
//				}
//			}
			yield break;
		}
		...
		protected void DrawCatalogueEntry (LibraryCatalogueEntry entry, int index) {
			...
			entry.RTDeliveryTime = UnityEditor.EditorGUILayout.FloatField ((float)entry.RTDeliveryTime);
			GUILayout.EndHorizontal ();
		}
	...
	public class LibraryCatalogueEntry { 
		...
		public double RTDeliveryTime = 30;
		public double WTOrderedTime = -1;
		public double WTPickUpTime = -1;
		public bool HasBeenPlaced {
			get {
				return WTOrderedTime > 0;
			}
		}
		public bool HasArrived {
			get {
				if (HasBeenPlaced && !HasBeenDelivered) {
					return WorldClock.Time > (WTOrderedTime + WorldClock.RTSecondsToGameSeconds (RTDeliveryTime));
				}
				return false;
			}
		}
