...
namespace Frontiers
{
	public class Skills : Manager
	{
		...
		protected void RefreshSkills ()
		{
			for (int i = 0; i < SkillList.Count; i++) {
				...
				#if DEBUG_SKILLS
				Debug.Log ("Refreshing skill " + newSkill.name);
				#endif
				...
				var associatedWorldItemNameEnum = associatedWorldItems.GetEnumerator ();
				while (associatedWorldItemNameEnum.MoveNext( )) {
					//using an enumerator so we can modify string
					string associatedWorldItemName = associatedWorldItemNameEnum.Current;
					if (!string.IsNullOrEmpty (associatedWorldItemName)) {
						...
						#if DEBUG_SKILLS
						Debug.Log ("Associating skill " + newSkill.name + " with worlditem name " + associatedWorldItemName);
						#endif
						associatedSkillsList.Add (newSkill);
					}
				}

				var associatedWIScriptNameEnum = associatedWIScripts.GetEnumerator ();
				while (associatedWIScriptNameEnum.MoveNext( )) {
					//using an enumerator so we can modify string
					#if DEBUG_SKILLS
					Debug.Log (newSkill.name + " Checking script name " + associatedWIScriptNameEnum.Current);
					#endif
					string associatedWIScriptName = associatedWIScriptNameEnum.Current;
					if (!string.IsNullOrEmpty (associatedWIScriptName)) {
						...
						#if DEBUG_SKILLS
						Debug.Log (newSkill.name + " associating with script name " + associatedWIScriptName);
						#endif
	...
	public class CredentialLevel
	{
		...
		[FrontiersBitMaskAttribute("GuildCredentials")]
		//TODO fix this
				public int
			Credentials;
